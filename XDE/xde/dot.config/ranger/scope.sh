...
case "$extension" in
	...
	# HTML Pages:
	htm|html|xhtml)
		have w3m    && w3m    -dump "$path" | head -n $maxln | fmt -s -w $width && exit 0
		have lynx   && lynx   -dump "$path" | head -n $maxln | fmt -s -w $width && exit 0
		have elinks && elinks -dump "$path" | head -n $maxln | fmt -s -w $width && exit 0
