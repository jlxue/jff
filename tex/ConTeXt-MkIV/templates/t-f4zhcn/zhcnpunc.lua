...
local function set_parameters(font, data)
   ...
   zhcnpunc_space = (data.zhcnpunc_space_factor / data.num) * quad
end

local dataset = {
   num = 1,
   zhcnpunc_space_factor = 0.55,
}

function zhcnpunc.proc (head, groupcode)
   ...
   for i, para in ipairs (paralist) do
	 ...
	 if zhchar == "full_width_close" then
	    ...
	    for j, v in ipairs (line) do
	       insert_node_after(head, v, make_glue_node(zhcnpunc_space, 0, 0))
	    end
	 end
	 line = nil
      end
